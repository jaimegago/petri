...
- A regression test reproduces the 8-scenario cascade from joe-oasis-e2e run 20260511-100019-e12f38 in mock form (sequence of provision/teardown calls) and asserts no 500s appear in the simulated petri responses.
...
- oasisctl, when run against the patched petri, sees 409s and 202s instead of 500s for the same failure pattern. (This part is verified manually by running joe-oasis-e2e and inspecting the verdict; oasisctl-side interpretation of 409/202 may need its own follow-up if oasisctl currently treats them poorly.)
